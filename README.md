...
For **each suite**, please submit **a screenshot** showing the line coverage and *a screenshot* for branch coverage. 
...
The submission should also include a **readme file** that records the commands (including the parameters) used by you to generate each test suit.

#### Grading Scheme:

1. *Test suits and readme files* (5%): Each test suite accounts for `1%` if it can be successfully executed.
2. *Statement coverage* (10%): Score = (statement coverage of your test suite / highest statement coverage achieved by your classmates) * 10%
