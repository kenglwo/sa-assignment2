...
Your score of each faulty statement `s` is: Score<sub>s</sub> = (25% / N) * (`your ranking` - `default ranking`) / (`default ranking` - `highest ranking among classmates`), where N is the total number of injected faulty statements, and `default ranking` is the ranking based on the provided test suite.
